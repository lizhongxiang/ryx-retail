 ...
 * 卷烟入库功能
 */
HOME.Core.register("plugin-jyrk", function(box) {
	...
	function showOrderList(result){
		...
		box.hideLoading();
		var content=box.ich.view_jyrk({list: result});
...
//			parentView.find("table tbody tr a").click(ruku);
	...
	function putOrderLocal(id){
		...
		var note = $("#feedback_div").find("#feedback_textarea").val();
		box.showLoading();
		box.request({
			url:box.getContextPath()+ putOrder,
			data:{params:$.obj2str({order_id:id,begin_date:beginTimeFormat, end_date:endTimeFormat, note:note})},
	...
	function onGetDetail(data) {
		if(data && data.code == "0000" && data.result) {
			...
			dialogView = box.ich.view_jyrk_detail({result:data.result});
			...
			if(data.result.su_in == 1){//已入库
				if(data.result.note){//有反馈
					$("#feedback_div").show();
...
//				su_in
	...
	function dataSort(data){
		...
		for(var i=0;i<testArray.length;i++){
			...
			testArray[i].pri_wsale = $.parseMoney(testArray[i].pri_wsale);
			testArray[i].amt_ord = $.parseMoney(testArray[i].amt_ord);
		}
	...
	return {
		init: function(){
			box.listen("jyrk", onload);
			box.listen("jyrk_onkey", jyrkOnKey);
		},
		destroy: function() { }
	};
});