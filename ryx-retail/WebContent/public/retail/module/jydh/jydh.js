...
HOME.Core.register("plugin-jydh", function(box) {
	...
	var paramsUrl = "retail/tobacco/getOrderParams";//订货参数
	var orderUrl = "retail/tobacco/getRecurrentOrder";//获取零售户本期订单
	...
	var accountBalanceUrl = "retail/tobacco/getAccountBalance";//获得余额
	var authenticateUrl = "retail/tobacco/authenticate";//验证登陆
	var loginDialog = null;
	var authenticateStatus = false;//验证登录状态（成功、失败
	...
	function loadJYDH(view) {
		...
		// 每次进入卷烟订货重新获取订货参数 为了解决过了订货时间按钮仍然可以点击的问题
//		if(params) {
//			getOrder();
//		} else {
			getAuthenticateInfo();
...
//		}
	...
	function getAuthenticateInfo() {
		...
		if (loginDialog) {
			var messageList = [];
			if(!$.validateForms(loginDialog, messageList)) {
				box.showAlert({message: messageList[0]});
				box.stopEvent(e);
				return;
			}
			authenticateParams.t_username = encode(loginDialog.find("#jydh_login_name").val());
			authenticateParams.t_password = encode(loginDialog.find("#jydh_login_pwd").val());
		}
		box.request({
			url: box.getContextPath() + authenticateUrl,
			data: {params: $.obj2str(authenticateParams)},
			success: function (data) {
				if (data.code = '0000') {
					authenticateStatus = true;
					if (loginDialog) {
						box.closeDialog({content: loginDialog});
					}
					getParams();
//					box.showAlert({message:"身份信息验证成功"});
				} else {
	...
	function onGetTobaccoList(data) {
		if(data && data.code == "0000") {
			...
			for(var i=0; i < len; i++) {
				...
				item.pri_wsale =  $.parseMoney(item.pri_wsale);
				...
				for(var j=0; j < len2; j++) {
					...
					if(item.item_id == item2.item_id) {
						...
						item.pri_wsale= $.parseMoney(item2.pri_wsale);
						item.amt_ord = $.parseMoney(item2.amt_ord);
						item.qty_lmt = item2.qty_rsn;
...
//						item.qty_whse = item.qty_whse < 0 ? 0 :item.qty_whse ;
						item.amt_ord = $.parseMoney(item.pri_wsale * item2.qty_ord);
						if(item2.qty_lmt) item.qty_lmt = item2.qty_lmt;
			...
			for(var j=0; j < selectTobacco.length; j++) {
				...
				if(order.flag == "0") {
					...
					if(item.qty_ord > item.qty_lmt) {
						...
						item.amt_ord = $.parseMoney(item.qty_ord * parseFloat(item.pri_wsale));
					}
				} else {
					item.reqlmt = true;
				}
	...
	function showOrder() {
		...
		parentView.find("#accountBalanceBtn").click(ontAccountBalance);//账户余额
	...
	function showTobaccoList() {
		/*	卷烟商品列表 金额格式 -----已做修改 onGetTobaccoList
		if(showTobacco && showTobacco.length>0) {
			for(var index=0; index<showTobacco.length; index++)  {
				showTobacco[index].pri_wsale = $.parseMoney(showTobacco[index].pri_wsale);
			}
		}
		*/
	...
	*/ 
	...
	function showItem(item) {
		...
		if(sp) {
			...
			else if(sp.top > 310) selectTable.parent().scrollTop(selectTable.parent().scrollTop() + sp.top - 40);
		}
	...
	function checkSetItemQty(req, qty) {
		...
		if(qty != currentItem.qty_ord_old && qty > currentItem.qty_lmt) {
			if(currentItem.qty_ord_old > currentItem.qty_lmt) {
				box.showConfirm({
					message: currentItem.item_name + "限量已调低为 "+currentItem.qty_lmt+"，修改订购量将减少订购，是否继续修改？",
					ok: function() {
	...
	function setItemQty(req, qty) {
		...
		currentItem.amt_ord = $.parseMoney(currentItem.qty_ord * parseFloat(currentItem.pri_wsale));
...
//								item.qty_whse = item.qty_whse < 0 ? 0 :item.qty_whse;
	...
	function onSubmitOrder(e) {
		...
		$.each(selectTable.find("tbody tr"), function(i, obj) {
			...
			var lmt = parseFloat($tr.attr("data-lmt"));
			var itemId = $tr.attr("data-id");
			if(qty > 0 || req > 0) {
				list.push({item_id: itemId, qty_req: req, qty_ord: qty, qty_lmt: lmt});
			}
			max += qty;
		});
		
		if(max > parseFloat(params.order_limit)) {
			box.showAlert({message:"订单数量超出总限量 "+(max - parseFloat(params.order_limit))+" 条，请调整您的订单"});
			submitButton.removeAttr("disabled");
			box.stopEvent(e);
			return;
		}
...
//							showSubmitOrderDialog(msg, nextstart, nextend);
							newItemIsEmpty = true;
						}
					}else{
						box.showAlert({message: "订单提交失败，请稍后重试！"});
	...
	//卷烟订货，提交提醒（下次订货日期，提交状态）-----------已经屏蔽
	function showSubmitOrderDialog (msg, beginDate, endDate){
		var data = {msg:msg, begin_date:beginDate, end_date:endDate};
		submitOrderDialog = box.ich.view_jydh_submit_order_dialog({data:data});
		box.showDialog({
			title:"提示",
			width:380,
			height:240,
			content: submitOrderDialog,
			close: function(e){
				//parentView.find("#addconsumer").removeAttr("disabled");
			}
		});
		submitOrderDialog.find("#submit_order_dialog_btn").unbind("click").click(function(e){//支付完成
			box.closeDialog({content: submitOrderDialog});
		});
	}
	...
	function showLoginDialog() {
		...
		loginDialog = box.ich.view_jydh_login();
		box.showDialog({
			title: "登录",
			width: 380,
			height: 295,
		...
		loginDialog.find("#loginSubmitBtn").unbind("click").click(getAuthenticateInfo);
		loginDialog.find("#loginCancelBtn").unbind("click").click(function () {
			box.closeDialog({content: loginDialog});
		});
	}
	
	//查询账户余额
	function ontAccountBalance() {
		box.request({
			url: box.getContextPath() + accountBalanceUrl,
			success: function (data) {
				if (data.code != '0000') {
					box.showAlert({message:"查询账户余额失败！"});
				}
				if (!data.result) {
					box.showAlert({message:"查询账户余额失败！"});
				}
				...
				box.showAlert({message:"您的账户余额为："+$.parseMoney(balance)+" 元"});
			},
			error: function () {
				box.showAlert({message:"查询账户余额失败！"});
			}
