 ...
 * 商品销售功能
 */
HOME.Core.register("plugin-spxs", function(box) {
	// 接口地址
...
//	var searchMerchConsumer = "retail/consumer/searchMerchConsumerInfo"; // 提交14
	...
	var total = null; //销售单数据：quantity商品总量, amount商品总额, amt_ss实收金额
	...
	var promotionIds = [];//用于提交销售单时，但现在已经改为：map(sql(type;key));---------测试是否有用
	var executorHistory = [];
	...
	function loadSPXS(view) {
		...
		parentView.empty().append(box.ich.view_spxs());
		...
		maxQty = parentView.find("#max_ord_qty");//数量合计
		maxAmt = parentView.find("#max_ord_amt");//金额合计
		maxDiscount = parentView.find("#max_ord_discount");//优惠金额
		orderSubmitButton = parentView.find("#order_submit");//结算按钮
		clearButton = parentView.find("#clear_all");
		//交班结算赋值
		allOrderSubmitButton = parentView.find("#all_order_submit");
		onlottery = parentView.find("#onlottery");
		
		itemBarInput.change(onItemBarInputChange);//商品条码
		choiceItemButton.click(onChoiceItemButtonClick);//选择商品
		currentItemQty.change(onQuantityInputChange);//数量
		currentItemAmt.change(onAmtInputChange);//金额

		orderSubmitButton.click(onOrderSubmitButtonClick);//结算
		...
		/*  ↖↗↘↙↓↑←→ */
		setHeaderTip();//顶部快捷键
		onlottery.click(onlotteryClick);//抽奖
		prepareParamAndRequestData(searchMerchPromotionUrl, {}, searchMerchPromotionSuccess);// 页面加载的时候获取促销活动列表
		
	}
	
	// 销售页面打开的时候获取促销活动列表
	function searchMerchPromotionSuccess(result) {
		proInf = result;//促销信息列表
		if(proInf){
			proInfMap = {};
	...
	function setHeaderTip(){//顶部快捷键
		/*  ↖↗↘↙↓↑←→ */
		//parentView.setHeaderTip("F5 选择会员　　F6 选择商品　　F9 结算　　F12 清空销售单　　↓↑ 选择销售单行　　Delete 删除选中行");
		//快捷键链接动作
		parentView.setHeaderTip("<ul id='view_spxs_headertip'>"+
		...
		parentView.closest(".window").find("#view_spxs_headertip").on("click","li",function(){			
			var id = $(this).attr("id");
			if(id=="onkey_f5"){
				consumerInput.focus();
...
//						selectItem(tr.data("item"), true);
	...
	function onlotteryClick(){//抽奖
		var lh = box.ich.view_spxs_lottery();
		lh = box.showDialog({
			content: lh,
			width: 300,
			height: 200,
	...
	//展示商品列表+促销信息(组件order数据)
	function getAllLineItem(){
		var itemInLine = null;
		var orderList = [];
		var order = {qty_ord_total: 0, amtys_ord_total: 0, qty_ord_count: 0};//商品销售数量，应收金额合计，销售商品种类
...
//			order.consumer_grade = currentConsumer.grade;
		...
		adjustedAmount = order.adjusted_amount;//单的促销金额
		promotionIds = order.promotion_ids;//单的促销id
	...
	function insertPromotionIcon(itemTr, itemInLine) {
		if(itemInLine.promotion_ids && proInf) {
			$.each(itemInLine.promotion_ids, function(key, values){
					...
					if(key==promotionId) {
						if(promotionType=="30") {
							//if(!itemTr.find("td:eq(1)").children().hasClass("projifen")){
								...
								itemTr.find("td:eq(1)").append("<span class='salepro_img projifen pageIcon pageIcon_jifen' alt='jifen' style='display:inline-block;' ></span>");	
							...
							if(!itemTr.find("td:eq(1)").children().hasClass("prozhe")){
								...
								itemTr.find("td:eq(1)").append("<span class='salepro_img prozhe pageIcon pageIcon_zhekou' alt='zhe' style='display:inline-block;' ></span>");
	...
	function updateCurrentItem(itemTr) {
		...
		if(currentItem && currentItem.line_num==selectedItem.line_num) {
...
//				itemTr.data("item", selectedItem);
		...
		if(order.promotion_ids){
			$.each(order.promotion_ids, function(key, values){
				for(var m=0;m<proInf.length;m++){
					//if(order.promotion_ids[n]==proInf[m].promotion_id){
					if(key==proInf[m].promotion_id){
						if(proInf[m].promotion_type=="40"){
							$(".footer_ul").find("li:eq(3)").find(".jian_img").css("display","inline-block");
						...
						if(proInf[m].promotion_type=="13"){
							$(".footer_ul").find("li:eq(3)").find(".vip_img").css("display","inline-block");
	...
	function onSearchDutyShiftSuccess(result) {//.unbind("click")
		...
		var lastShiftDate = new Date(lastShiftYear, lastShiftMonth-1, lastShiftDay, lastShiftHour, lastShiftMinute, lastShiftSecond); 
		...
		var shiftDate=new Date(shiftYear, shiftMonth-1, shiftDay, shiftHour, shiftMinute, shiftSecond);
		result.shift_date=shiftDate.format("yyyy-MM-dd hh:mm:ss");
		result.amt_sale_card = $.parseMoney(result.amt_sale_card);
		result.amt_sale_cash = $.parseMoney(result.amt_sale_cash);
		result.amt_return = $.parseMoney(result.amt_return);
		//显示数据
		selectAllSubmitSaleOrder = box.ich.view_spxs_allOrderSubmit({result:result});
		...
		$("#closeallordersubmit").click(function(e) {
			...
			prepareParamAndRequestData(addsubmitSaleOrderUrl, params, addsubmitSaleOrderSuccess);
		});
	...
	function addsubmitSaleOrderSuccess(result) {
	...
	function onChoiceItemButtonClick(e) {
		box.controls.choiceItemDialog(undefined, function(itemList) {
			showItem(itemList);
		}, currentItemQty, choiceItemButton, "spxs");
//		var ;
	}
	...
	function resetConsumer() {
		currentConsumer = {};
		$(".vip_img").hide();
		consumerContainer.empty().append(box.ich.view_spxs_consumer_input2());
...
//			prepareParamAndRequestData(searchMerchConsumer, {telephone: keyword, page_index: -1, page_size: -1}, onSearchConsumerSuccess);
	...
	function onSearchConsumerSuccess(result) {
		if(result.length>0) {
			...
			$("#input_consumer").val(currentConsumer.consumer_name);
			$("#clean_consumer_a").css("display", "block");
//			var consumerUl = box.ich.view_spxs_consumer_ul({consumer:currentConsumer});
...
//			box.console.log("--添加会员---: consumer_grade："+currentConsumer.grade+"==consumer_point："+currentConsumer.curscore);
//			box.console.log("--添加会员---: "+ $.obj2str(keXianPinObj));
			consumerContainer.find("#clean_consumer_a").unbind("click").click(onClearConsumerButtonClick);
			box.console.log("添加会员：total："+$.obj2str(total));
			box.console.log("添加会员：currentItem："+$.obj2str(currentItem));
		} else {
			consumerContainer.find("#input_consumer").val("");
	...
	function onClearConsumerButtonClick(e) {
		...
		refreshTotalContainer();
		keXianPinObj.consumer="";
		$("#clean_consumer_a").css("display", "none");//将取消会员符号隐藏
		$(".vip_img").hide();//将底部的vip符号隐藏
		box.com.commSend({type: "showall", message: (keXianPinObj)});
//		box.console.log("--取消会员---: "+ $.obj2str(keXianPinObj));
		box.console.log("取消会员：total："+$.obj2str(total));
		box.console.log("取消会员：currentItem："+$.obj2str(currentItem));
	}
...
//		box.console.log(result);
		if(result && result.length>0) {
			...
			if(!isBase) {
				if(result[0].status=="0"){
					box.showAlert({message:result[0].item_name+"等"+(result.length-itemList.length)+"个商品已删除！<br/>请在商品管理中恢复后再进行销售！"});
					itemBarInput.val("");
	...
	function showConfirmDialog(merchItemObject, confirmInfo, backInfo) {
		var confirmDialog = box.ich.view_spxs_confirm_dialog({context:confirmInfo});
		confirmDialog = box.showDialog({
			...
			buttons: [{
	            ...
	            click: function(){
	            	...
	            	box.request({
						url : box.getContextPath() + recoverItemUrl,
	...
	function showItem(itemList) {
		if(itemList.length>0) {
			if(itemList.length==1) {
				...
				selectItemDialog = box.ich.view_spxs_select_item_dialog({list: itemList});
	...
	function onQuantityInputChange(e) {
		if(currentItem) {
			...
			checkItemAmtOrd(currentItem, value, 1,  function() {
				currentItem.qty_ord = value;
				
				var amt = 0.0;
				if(currentItem.pri) {
					amt=currentItem.pri*currentItem.qty_ord;
				} else {
				...
				$(e.currentTarget).val(currentItem.qty_ord);
			});
		}
	}
	...
	function onAmtInputChange(e) {
		if(currentItem) {
			...
			// 检查价格预警
			checkItemAmtOrd(currentItem, value, 2, function() {
...
//				refreshTotalContainer();
	...
	function onDeleteItem(data) {
		...
		box.com.commSend({type: "showall", message: (keXianPinObj)});
//		box.console.log("--删除商品---: "+ $.obj2str(keXianPinObj));
...
//		box.console.log("--679商品(3)---promotionItem: "+$.obj2str(promotionItem));
		box.console.log("删除商品：total："+$.obj2str(total));
		box.console.log("删除商品：currentItem："+$.obj2str(currentItem));
		refreshLineLabel();
	}
	
	//向页面添加商品
	function addItem(item) {
		item.line_num = tableBody.children().length + 1;
		item.qty_ord = 1; //销售数量
		item.amt_ord = $.parseMoney(parseFloat(item.big_pri4)*parseFloat(item.discount)/100.0);//商品折后金额
		item.pri = $.parseMoney(parseFloat(item.big_pri4));
		var tr = box.ich.view_spxs_item_row(item);
		tableBody.append(tr);
		parentView.find("#right_container").scrollTop(99999);
		tableBody.find("tr[data-num="+item.line_num+"]").data("item", item);
		getAllLineItem();//展示商品列表+促销信息
		selectItem(item);
		refreshTotalContainer();
		keXianPinObj.operate = "add";
		keXianPinObj.operate_line = item.line_num;
		box.com.commSend({type: "showall", message: (keXianPinObj)});
//		box.console.log("--添加商品---: "+ $.obj2str(keXianPinObj));
		box.console.log("添加商品：total："+$.obj2str(total));
		box.console.log("添加商品：currentItem："+$.obj2str(currentItem));
	}
	
	//单击商品（tr）------在展现左侧商品详细信息栏中展示商品
	function selectItem(data, onselect) {
		...
		if(currentItem) {
			...
			if(onselect) {
				...
				box.com.commSend({type: "showall", message: (keXianPinObj)});
//				box.console.log("--单机商品---: "+ $.obj2str(keXianPinObj));
...
//				box.console.log("--选择商品(1)---keXianPinObj:"+$.obj2str(keXianPinObj));
		...
		box.console.log("选择商品：total："+$.obj2str(total));
		box.console.log("选择商品：currentItem："+$.obj2str(currentItem));
	}
...
//		//refreshTrItem(currentItem);
	...
	function setKxpItemList(data){
		...
		kxpItemMap.pir4 = $.parseMoney(data.pri);
		...
		if(data.promotion_ids){
			var promotion_descs = [];
			var kxpPromotionIds = [];
			$.each(data.promotion_ids, function(key, values){
				promotion_descs.push(values.promotion_key);
				kxpPromotionIds.push(key);
			});
...
//			}
			...
			kxpItemMap.promotion_ids = kxpPromotionIds ;
		}else{
	...
	function setKxpObj(){//客显屏单赋值
		...
		if(promotionItem.promotion_ids){
			...
			var kxpPromotionIds = [];
			$.each(promotionItem.promotion_ids, function(key, values){
				promotion_descs.push(values.promotion_key);
				kxpPromotionIds.push(key);
			});
			
			keXianPinObj.promotion_descs = promotion_descs ;
			keXianPinObj.promotion_ids = kxpPromotionIds ;//----------------促销id
		}else{
	...
	function refreshTotalContainer() {
		...
		//单合计金额--应收
//		total.amount = $.parseMoney(promotionItem.amtys_ord_total - (promotionItem.adjusted_amount ? promotionItem.adjusted_amount : 0));
		total.amount = 0.0;
		//单总金额：未折扣
		...
		total.amount = $.parseMoney(total.amount - (promotionItem.adjusted_amount ? promotionItem.adjusted_amount : 0));
		...
		if(currentConsumer.consumer_id && currentConsumer.discount){
			total.consumer_rebate_amount = $.parseMoney(parseFloat(total.amount) * (1.0-parseFloat(currentConsumer.discount) / 100));//会员折扣
			total.amount = $.parseMoney(parseFloat(total.amount) * parseFloat(currentConsumer.discount) / 100);//会员价
		}
		maxQty.text(total.quantity);//数量合计
		maxAmt.text($.parseMoney(total.amount));//金额合计
		maxDiscount.text($.parseMoney(total.oriAmount-total.amount));//优惠金额
		if(submitDialog) {
		...
		if(currentItem && currentItem.qty_ord) {
			box.com.commSend({type: "showitem", message: {current_item:currentItem.item_name, current_qty:currentItem.qty_ord, current_amt:$.parseMoney(currentItem.amt_ord), total_qty:currentItem.qty_ord, total_amt:$.parseMoney(total.amount)}});
//			box.console.log("--页面变动---: "+ $.obj2str({current_item:currentItem.item_name, current_qty:currentItem.qty_ord, current_amt:$.parseMoney(currentItem.amt_ord), total_qty:currentItem.qty_ord, total_amt:$.parseMoney(total.amount)}));
		}
	}
	
	function refreshLineLabel() {//重新排序
		$.each(tableBody.children(), function(i, obj) {
			$(obj).find(".linelabel").text(i+1);
			$(obj).attr("data-num",i+1);
			$(obj).data("item").line_num = i+1;
		});
	}
	...
	function onOrderSubmitButtonClick(e) {
		...
		var consumerId = null;
		var consumerDis = "100%";
		currentConsumer.add_score = "0";
		total.amount = maxAmt.text();
		total.input_amount = total.amount ;
		...
		//店铺优惠金额
		box.com.commSend({type: "showorder", message: {
			total_qty:total.quantity, total_amt:$.parseMoney(total.amount), amt_ss:$.parseMoney(total.amount), 
			amt_left:$.parseMoney(0)}});
...
//		var promotionMoney = $.round( parseFloat(total.oriAmount) - parseFloat(total.amount), 2);
		if(currentConsumer && currentConsumer.consumer_id) {
			...
			if(!amountPerPoint || amountPerPoint == 0){
				...
				currentConsumer.add_score = amountPerPoint ? parseInt(total.amount / amountPerPoint) : 0;
			}
		}
		
		submitDialog = box.ich.view_spxs_cash_submit_dialog({
			consumer_id:consumerId, consumer_point: currentConsumer.add_score, consumer_grade_discount:consumerDis, 
			totalAmount: $.parseMoney(total.oriAmount), discountVal:$.parseMoney(total.oriAmount-total.amount), 
			receivable:$.parseMoney(total.amount), change: $.parseMoney(0)});
		...
		receivableInput = submitDialog.find("#receivable_input");//应收
		actualInput = submitDialog.find("#actual_input");//实收
		cashSubmitButton = submitDialog.find("#cash_submit_button");//现金支付按钮
		cardSubmitButton = submitDialog.find("#card_submit_button");//刷卡按钮
		hangingSubmitButton = submitDialog.find("#hanging_submit_button"); // 挂账按钮
		actualInput.select();
		receivableInput.keyup(onSettlementAmtKeyUp);
		receivableInput.change(setBtnDisable);
		actualInput.change(setBtnDisable);
		actualInput.keyup(onSettlementAmtKeyUp);
		//刷卡消费按钮 
...
//		hangingSubmitButton.unbind("click").click(onSubmitButtonClick(true,3)); // 挂账
	...
	function setBtnDisable(){
		if (!receivableInput.val()) {
			box.showAlert({message: "请输入应收金额！"});
			cashSubmitButton.attr("disabled", "disabled"); 
			cardSubmitButton.attr("disabled", "disabled");
		} else if (!actualInput.val() ) {
			box.showAlert({message: "请输入实收金额！"});
			cashSubmitButton.attr("disabled", "disabled"); 
			cardSubmitButton.attr("disabled", "disabled");
		} else if (parseFloat(actualInput.val()) < parseFloat(receivableInput.val())) {
			box.showAlert({message: "实收金额不能小于应收金额，系统将自动赋值为"+$.parseMoney(total.input_amount)});
			total.amt_ss = $.parseMoney(total.input_amount);
			changeSettlement();
			actualInput.val($.parseMoney(total.input_amount));
		}
	}
	
	//结算界面，应收、实收，改变值时，改变找零
	function onSettlementAmtKeyUp(e) {
		
		if(receivableInput.val() && actualInput.val() ){
			cashSubmitButton.removeAttr("disabled");
			cardSubmitButton.removeAttr("disabled");
		}
		
		total.input_amount = receivableInput.val();//输入的应收金额
		total.amt_ss = actualInput.val();//实收金额
		
		if(isNaN(parseFloat(total.input_amount))) {
			total.input_amount = 0;
		}
		
		//输入应收金额不能大于实际应收金额
		if (parseFloat(total.input_amount) > parseFloat(total.amount)) {
			receivableInput.val(total.input_amount.substring(0, total.input_amount.length-1));
			box.showAlert({message: "输入应收金额不能大于实际应收金额！"});
			return;
		}
		
		changeSettlement();
		
		keXianPinObj.adjusted_amt = $.parseMoney(parseFloat(total.oriAmount) - parseFloat(total.amount) + parseFloat(total.mo_ling));
		box.com.commSend({type: "showall", message: (keXianPinObj)});
		box.com.commSend({type: "showorder", message: {total_qty:total.quantity, total_amt:$.parseMoney(total.input_amount), 
						  amt_ss:$.parseMoney(total.amt_ss), amt_left:$.parseMoney(total.change)}});
//		box.console.log("--结算界面---: "+ $.obj2str({total_qty:total.quantity, total_amt:$.parseMoney(total.amount), 
//			  amt_ss:$.parseMoney(total.amt_ss), amt_left:$.parseMoney(total.change)}));
		
	}
	
	//* 1、修改会员积分
	//* 2、修改抹零，实应收》原应收 ，算入利润；实应收《原应收，算入优惠
	//* 3、修改找零
	//* 4、修改优惠金额
	function changeSettlement(){
		if (total.input_amount.isNAN) {
			total.input_amount = $.parseMoney(0);
		}
		
		//1、修改抹零
		//实际应收 >= 输入应收，算入优惠
		if (parseFloat(total.amount) >= parseFloat(total.input_amount)) {
			total.mo_ling = $.parseMoney(parseFloat(total.amount) - parseFloat(total.input_amount));
		} else {
			total.mo_ling = $.parseMoney(0);
		}
		$("#moLingAmt").html(total.mo_ling );
		
		//2、修改会员积分
		if(currentConsumer && currentConsumer.consumer_id ) {
			var amountPerPoint = box.user.amount_per_point;//积分规则
			//会员积分
			if(!amountPerPoint || amountPerPoint == 0){
				currentConsumer.add_score = "0";
			}else{
				currentConsumer.add_score = amountPerPoint ? parseInt(total.input_amount / amountPerPoint) : 0;
			}
			$("#consumer_point").html(currentConsumer.add_score);
		}
		
		//3、修改找零：实收-应收
		total.change = $.parseMoney(parseFloat(total.amt_ss) - parseFloat(total.input_amount)); //找零
		submitDialog.find("#change").text(total.change);
		
		//4:修改总优惠金额
		$("#discountVal").text($.parseMoney(parseFloat(total.oriAmount) - parseFloat(total.amount) + parseFloat(total.mo_ling)) );
	}
	...
	function onSubmitButtonClick(isCash,payType) {
		...
		return function(e) {
			...
			checkAmtOrd(total.amt_ss, total.input_amount, function() {
				onSubmitClick(isCash,payType);
			}, function() {
				// 点击取消，不做动作
				cashSubmitButton.removeAttr("disabled");
				cardSubmitButton.removeAttr("disabled");
	...
	//组件提交销售单数据
	function setOrderParams(isCash,payType){
		itemList.length=0;
		$.each(tableBody.children(), function(i, obj) {
			var data = $(obj).data("item");
			data = $(obj).data("item");
			var itemData = {};
			itemData.item_id = data.item_id;
			itemData.item_bar = data.item_bar;
			itemData.cost = data.cost;
			itemData.item_kind_id = data.item_kind_id;
			itemData.line_num = data.line_num;
			itemData.item_name = data.item_name;
			if(data.promotion_ids){
				itemData.promotion_ids = data.promotion_ids;
			}
			itemData.item_unit_name = data.item_unit_name;
			itemData.pri4 = data.pri4;
			itemData.big_bar = data.big_bar;
			itemData.big_unit_name = data.big_unit_name;
			itemData.big_pri4 = data.big_pri4;
			itemData.unit_ratio = data.unit_ratio;
			itemData.pack_id = data.seq_id;
			itemData.sale_quantity = data.qty_ord;
//			itemData.sale_amount = data.amt_ord;//折扣金额
			data.adjusted_price = data.adjusted_price  ? data.adjusted_price : 0;
//			itemData.sale_amount = (parseFloat(data.newPri) + parseFloat(data.adjusted_price)) * parseFloat(data.qty_ord);//未折扣金额
			if (data.adjusted_amount) {
				itemData.sale_amount = parseFloat(data.amt_ord) + parseFloat(data.adjusted_amount);
			} else {
				itemData.sale_amount = parseFloat(data.amt_ord) ;
			}
			itemData.line_rebate_amount = 0;
			if(data.adjusted_amount){
				itemData.line_rebate_amount = data.adjusted_amount;
			}
			
			if(!data.cost) {
				delete(data.cost);
				delete(itemData.cost);
			}
			itemList.push(itemData);
		});
		var obj = {total_amount:total.amount, input_received_amount:total.input_amount, received_amount:total.amt_ss, list: itemList};
		
//		var obj = {qty_ord_total: total.quantity, qty_ord_count: total.count, 
//				amt_ord_total: total.amt_ss, amtys_ord_total: total.amount, 
//				adjusted_amount: total.oriAmount-total.amount, 
//				order_type: "01", operator: "1", list: itemList};
		
		if(currentConsumer && currentConsumer.consumer_id) {//会员编号
			obj.consumer_id = currentConsumer.consumer_id;
			obj.consumer_rebate_amount = total.consumer_rebate_amount;//会员让利金额
			obj.consumer_point = currentConsumer.add_score;//会员积分
		}
		
		obj.total_rebate_amount = (total.oriAmount-total.amount).toFixed(3);//整单让利
		if(promotionIds) {
			obj.promotion_ids = promotionIds;//促销信息
		}
		obj.change_amount = (parseFloat(total.amt_ss) - parseFloat(total.input_amount)).toFixed(3);//找零
		return obj;
	}
	
	function onSubmitClick(isCash,payType) {
		var obj = setOrderParams(isCash,payType);//组件销售单数据
		var url = "";
//		return;
		if(isCash) {//现金支付url
			if(payType==1){
			...
			if(orderInfo && parseFloat(orderInfo.amtys_ord_total) == parseFloat(total.input_amount )){
				submitCardOrder(orderInfo, submitDialog.find("#actual_input").val());
				return ;
			}
			else{
				url = submitSaleOrderWithCardUrl;//刷卡结算
			}
		}
		...
		box.request({
			...
			success: function(json){
				if(json && json.code=="0000") {
					...
					orderInfo.moling = parseFloat(orderInfo.amt_ord_loss);
					// 挂账的找零和实收都为0, 在后台还会处理抹零
					if(payType==3) {
						orderInfo.change = $.parseMoney(0);
						orderInfo.amt_ord_total = $.parseMoney(0);
					} else {
						orderInfo.change = $.parseMoney(orderInfo.amt_ord_total-orderInfo.amtys_ord_total);//找零
						orderInfo.amt_ord_total = $.parseMoney(orderInfo.amt_ord_total);//实收
					}
					orderInfo.amtys_ord_total = $.parseMoney(orderInfo.amtys_ord_total);//输入应收
					orderInfo.card_id = currentConsumer.card_id;//会员编号
	...
	function submitCardOrder(order, amt_ss){//刷卡支付
		...
		var consumeSuccessByCardOrCardToCash = function(result){
			...
			if(orderInfo.pay_type == 1){
				title = "现金结算";
				cardConfirmDialog = box.ich.view_spxs_cash_confirm_dialog({order:orderInfo});
			}
			else{
				title = "刷卡结算成功";
				cardConfirmDialog = box.ich.view_spxs_card_confirm_dialog({order:orderInfo});
			}
			...
			cardConfirmDialog = box.showDialog({
				...
				onkey: function(data) {
					if(data.key == HOME.Keys.ENTER || data.key == HOME.Keys.TAB || data.key == HOME.Keys.SPACE || data.key == HOME.Keys.LEFT || data.key == HOME.Keys.RIGHT || data.key == HOME.Keys.UP || data.key == HOME.Keys.DOWN) {
						clearTimeout(orderInfoCloseTimeout);
						box.closeDialog({content: cardConfirmDialog});
					}
					if(data.key == HOME.Keys.BARCODE){
						clearTimeout(orderInfoCloseTimeout);
						box.closeDialog({content: cardConfirmDialog});
						getItem(data.code);
					}
			...
			*/
			...
			if(itemList) {
				for(var i=0; i<itemList.length; i++) {
					var item = itemList[i];
					var printedItem = {};
					printedItem.item_name = item.item_name;
					printedItem.qty_ord = item.sale_quantity;
					printedItem.amt_ord = $.parseMoney(item.sale_amount - item.line_rebate_amount);
					printedItem.pri = $.parseMoney(printedItem.amt_ord / item.sale_quantity);
					printedItemList.push(printedItem);
				}
			}
			box.controls.print(order, total, printedItemList, currentConsumer);
			cardConfirmDialog.find("#printing").empty().append($('<input id="close_button" type="button" class="minor_button" style="width:80px; height:30px; font-size:14px;margin-top:5px;" value="关 闭"/>'));
			$("#close_button").focus();
			cardConfirmDialog.find("#close_button").unbind("click").click(closeOrderInfoCardDialog);
			orderInfoCloseTimeout = setTimeout(closeOrderInfoCardDialog, 20000);
		};
		//刷卡转现金支付   ----点击现金支付时执行
		var submitCardToCash = function(){
			var amt_ord_total = submitDialog.find("#actual_input").val();
			var amtys_ord_total = total.amount; 
			var amt_ord_change = $.parseMoney(parseFloat(amt_ord_total) - parseFloat(amtys_ord_total)); //找零AMT_ORD_CHANGE
			...
			prepareParamAndRequestData(cashPaySaleOrderUrl, params, consumeSuccessByCardOrCardToCash);
		};
		//刷卡之后回调函数
		var onsumeUseCardCallback = function(data) {
			...
			if(data){
				...
				var amt_ord_total = submitDialog.find("#actual_input").val();
				var amtys_ord_total = total.input_amount; 
				var amt_ord_change = $.parseMoney(parseFloat(amt_ord_total) - parseFloat(amtys_ord_total)); //找零AMT_ORD_CHANGE
				orderInfo.amtys_ord_total=amtys_ord_total;
				orderInfo.amt_ord_total=amt_ord_total;
				orderInfo.change=amt_ord_change;
		...
		box.showMask({
			...
			cancelButton:{},
			feeContent:{"hidden":false,val:$.parseMoney(order.amtys_ord_total)}
		});
		//box.showMask({message: "请在POS机上刷卡",isShowHiddenButton:true,buttonVal:"已支付完成",callBack:onsumeUseCardCallback});
		box.stopEvent();
		box.com.consume({num:2,pri:$.parseMoney(order.amtys_ord_total)},onsumeUseCardCallback);
	}
	
	//现金支付
	function submitCashOrder(orderInfo) {
		orderInfoDialog = box.ich.view_spxs_cash_confirm_dialog({order:orderInfo});
		...
		box.controls.print(orderInfo, total, printedItemList, currentConsumer);
		$("#order_submit").blur();
		
		//box.controls.print(orderInfo, total, itemList, currentConsumer);
		//$("#order_submit").blur();
		
		orderInfoDialog.find("#printing").empty().append($('<input id="close_button" type="button" class="minor_button" style="width:80px; height:30px; font-size:14px;margin-top:5px;" value="关 闭"/>'));
		$("#close_button").focus();
		orderInfoDialog.find("#close_button").unbind("click").click(closeOrderInfoDialog);
		
		orderInfoCloseTimeout = setTimeout(closeOrderInfoDialog, 20000);
...
//		box.console.log("--清空---: "+ $.obj2str("clearorder"));
...
//					selectItem(tr.data("item"), true);
	...
	function getShouldElementFromLine(promotion, line) {
		...
		if(promotion.promotion_should) {
			...
			if(shouldPromotion.seq_id && shouldPromotion.seq_id[line.seq_id]) {
				shouldElement = shouldPromotion.seq_id[line.seq_id];}
			else if(shouldPromotion.item_kind_id && shouldPromotion.item_kind_id[line.item_kind_id]) {
				shouldElement = shouldPromotion.item_kind_id[line.item_kind_id];}
	...
	function getPromotionExecutor(promotions) {
		var executor=null;
		//循环促销信息列表，//10:折扣, 13:会员折扣, 20:组合, 30:积分, 40:满减, 50:抽奖
...
//	function ItemDiscountPromotionExecutor(promotion) {//商品折扣
//		PromotionExecutor.call(this, promotion);//用PromotionExecutor这个方法替代当前方法，promotion是传过去的值
//		var strategy = function LinePromotionStrategy(executor) {
//			this.promote = function(order) {
//				for(var i=0; i<order.list.length; i++) {
//					var line = order.list[i];
//					var isMatch = function(line) {
//						var shouldElement = function getShouldElementFromLine(promotion, line) {
//							var shouldElement = null;
//							if(promotion.promotion_should) {
//								var shouldPromotion = promotion.promotion_should;
//								if(shouldPromotion.seq_id && shouldPromotion.seq_id[line.seq_id]) {
...
//							if(!line.notPromote){
//								if(!line.adjusted_price) line.adjusted_price = 0;
//								if(shouldElement.discount_price) {
//									line.adjusted_price = line.big_pri4 - shouldElement.discount_price;
//								} else if(shouldElement.discount) {
//									line.adjusted_price = line.big_pri4 * (1.0 - shouldElement.discount / 100);
//								}
//							}
//							if(!line.promotion_ids) line.promotion_ids = {};
//							var promotinMap ={};
//								promotinMap.promotion_type = this.type;//类型
//								promotinMap.promotion_key = line.adjusted_amount;//促销优惠金额
//							var pId = this.promotionId;
//							line.promotion_ids[pId] = promotinMap;
...
//			};
//		}
//		//this.loadStrategy(new LinePromotionStrategy(this));//值
//		this.loadStrategy = function(strategy) {
//			this.promotionStrategy = strategy;
//		};
...
//	}
	...
	function ItemDiscountPromotionExecutor(promotion) {
		//用PromotionExecutor这个方法替代当前方法，promotion是传过去的值
...
//					promotinMap.promotion_type = this.type;//类型
//					promotinMap.promotion_key = line.adjusted_amount;//促销优惠金额
//					alert(this.type+"-----"+line.adjusted_amount);
//				var pId = this.promotionId;
//				line.promotion_ids[pId] = promotinMap;
				return true;
			}
...
//				line.promotion_ids.push(this.promotionId);
				...
				var promotinMap ={};
					promotinMap.promotion_type = this.type;//类型
					promotinMap.promotion_key = line.adjusted_point;//促销优惠金额
					//alert(this.type+"----"+order.adjusted_point);
				var pId = this.promotionId;
				line.promotion_ids[pId] = promotinMap;
				return true;
			}
...
//				order.promotion_ids.push(this.promotionId);
				...
				var promotinMap ={};
					promotinMap.promotion_type = this.type;//类型
					promotinMap.promotion_key = order.adjusted_amount;//促销优惠金额
				var pId = this.promotionId;
				order.promotion_ids[pId] = promotinMap;
				return true;
			}
			return false;
		};
	...
	function CeilingReductionPromotionExecutor(promotion) {
		...
		this.matchOrder = function(order) {
			...
			if(reduction) {
				...
				var promotinMap ={};
					promotinMap.promotion_type = this.type;//类型
					promotinMap.promotion_key = reduction;//促销优惠金额
				var pId = this.promotionId;
				order.promotion_ids[pId] = promotinMap;
				return true;
			}
		...
		this.promote = function(order) {
			for(var i=0; i<order.list.length; i++) {
				...
				if(isMatch) {
					...
					if(!line.promotion_ids) line.promotion_ids = {};
					var promotinMap ={};
						promotinMap.promotion_type = executor.type;//类型
						promotinMap.promotion_key = line.adjusted_amount;//促销优惠金额
					var pId = executor.promotionId;
					line.promotion_ids[pId] = promotinMap;
				}
			}
		};
	 ...
	 * c:卷烟单品销售数量大于500
	 */
	function checkItemAmtOrd(item, val, type, ok, cancel) {
		...
		var amt = parseFloat(item.big_pri4) * parseFloat(item.qty_ord);
		var saleItemCost = parseFloat(item.cost) * parseFloat(item.qty_ord) * parseFloat(item.unit_ratio);
		val = parseFloat(val);
		if(amt) {
			var isWarn = false;
			...
			if (type == 1) {//修改数量
				if(item.item_kind_id == "01") {
					if (val > 500) {
//						warnMsg += "您输入的商品 数量大于500，超出异常预警！<br/>";
						warnMsg = "您输入的商品数量超出异常预警，是否确定修改？";
						isWarn = true;// 卷烟销售异常（销售数量大于500
					}
				}
			} else {//修改金额
				if(item.item_kind_id == "01") {
					if(val > amt * 1.5 || val < amt / 1.5) {
//						warnMsg += "您输入的商品 金额 大于原金额1.5倍，超出异常预警！<br/>";
						warnMsg = "您输入的商品金额超出异常预警，是否确定修改？";
						isWarn = true;// 卷烟销售异常(销售金额大于原金额1.5倍，或小于1.5倍
					} else if (saleItemCost > val) {
//						warnMsg += "您输入的商品金额 小于成本价，超出异常预警！<br/>";
						warnMsg = "您输入的商品金额超出异常预警，是否确定修改？";
						isWarn = true;// 卷烟销售异常（销售金额小于成本
					} else if (val > saleItemCost * 3) {
//						warnMsg += "您输入的商品金额 大于3倍成本，超出异常预警！<br/>";
						warnMsg = "您输入的商品金额超出异常预警，是否确定修改？";
						isWarn = true;// 卷烟销售异常（销售金额大于成本的3倍
					} 
				} else {
					if(val > amt * 2 || val < amt / 2 ) {
//						warnMsg += "您输入的商品金额 小于原金额1.5倍，超出异常预警！<br/>";
						warnMsg = "您输入的商品金额超出异常预警，是否确定修改？";
						isWarn = true;// 非烟销售异常(商品输入的销售价(商品销售) > 设定的销售价(商品管理)*2 或 设定销售价*2
					}
				}
			}
//			warnMsg += "是否确定修改";
...
//					message: "您输入的商品金额超出异常预警，是否确定修改？", 
	...
	return {
		init: function(){
			box.listen("spxs", loadSPXS);
			box.listen("spxs_close", onClose);
			box.listen("spxs_onkey",onKey);
		},
		destroy: function() { }
	};
});
