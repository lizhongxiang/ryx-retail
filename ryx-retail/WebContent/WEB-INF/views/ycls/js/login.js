...
$(document).ready(function() {
	...
	function setClertNum(){
		if(clertNum.val()) {
			cnprompt.css("display", "none");
		} else {
			cnprompt.css("display", "block");
		}
	}
	setUnPrompt();
	setPwPrompt();
	...
	clertNum.focusin(function() {
		cnprompt.css("display", "none");
	}).focusout(function(){
		setClertNum();
	}).keypress(function() {
		clertNum.removeClass("validate");
		cnprompt.css("display", "none");
	});
	...
	loginform.submit(function() {
		...
		var sf = serviceFlag.val().trim();
		var cn = clertNum.val().trim();
		var b = true;
		if(!un) {
			b = false;
			username.focus();
		...
		if(b) {
			...
			//setCookie("clerkNum",cn);
			if(cn && cn != ""){
				un=un+"_"+cn;
			}
			vc = vc.toUpperCase();
			//var params = {username: un, password: MD5(MD5(pw)+vc), challenge: vc, requestType: "ajax"};
			var params_my = {"serviceFlag":sf,"password":MD5(pw),"username": un,"validateCode":vc,"requestType":"ajax"};
	...
	function onLoginSuccess(data) {
		if(data.code=="0000"){
			//setCookie("accessToken",data.result.tokenSecret);
			var cb = callbackURL.val().trim();
//			cb = cb+"?"+"onceFlag="+onceFlag+"&tokenSecret="+data.result.tokensecret;
